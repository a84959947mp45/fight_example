...
./gogui-1.4.9/bin/gogui-twogtp -black "${BLACK_PROGRAM}" -white "${WHITE_PROGRAM}" -auto ${FORCE} -games ${TOTAL_GAMES} -sgffile ${SGF_PREFIX_NAME} -size ${BOARD_SIZE} -threads ${NUM_FIGHT} -verbose -alternate
 ./gogui-1.4.9/bin/gogui-twogtp -analyze $SGF_PREFIX_NAME.dat -force
chmod -R ${permissions} fight_results/${FIGHT_NAME}
# ==========================================================================================
